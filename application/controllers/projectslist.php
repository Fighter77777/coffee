...
class ProjectsList extends CI_Controller {
	...
	public function index($count_on_pg=NULL, $s_txt='search_text', $s_resp='responsible', $s_stat='status', $s_cat='category',$s_my_pr='only_my',$order='sorting',$order_direction='direction')
	...
	{				
		...
		if($entered){
			$this->load->model(array('users','projects','ctrl'));
			...
			$s_cat_db =($s_cat!='category')?$s_cat:NULL;
			...
			$body_dat['cat']=$this->projects->getCatList();			
			
			if(!empty($_POST)){
				//var_dump($_POST);
				$terms=$_POST;		//фільтри для запита вибрані користувачем				
			}else{			
				$terms=array('search_txt'=>$s_txt_db, 'search_responsible'=>$s_resp_db, 'search_status'=>$s_stat_db, 'search_category'=>$s_cat_db, 'only_my'=>$s_my_pr_db);
			}			
			
			$body_dat['sel_resp_v']=$body_dat['sel_stat_v']=$body_dat['sel_cat_v']='<все>';
			...
			if(!empty($terms['search_category'])) {
				$body_dat['sel_cat_id']=$s_cat=(int)$terms['search_category'];
				$body_dat['sel_cat_v']=$body_dat['cat'][$terms['search_category']];
			}			
			if($terms['only_my']==1) {
				$body_dat['only_my']=1;
				$s_my_pr='only_my';
			...
			//пагінація			
			$config['total_rows'] = $this->projects->count_all($terms);			
			//echo $_POST['tsk_in_page'];
			if(empty($count_on_pg) && empty($_POST['tsk_in_page'])) $count_on_pg=$this->ctrl->constDB('projects_num_pg');
			...
			$sort_url= base_url()."index.php/projectslist/index/{$config['per_page']}/{$s_txt}/{$s_resp}/{$s_stat}/{$s_cat}/{$s_my_pr}/";	//для початку ссилкі сортування
			...
			$body_dat['table']=$this->projects->dataForTabProj($config['curr_pg'],$config['per_page'],$terms,$order_db,$order_direction_db);	//масив - список проектів	
			...
			$body_dat['sort_category'] =$this->sort_url('category',$sort_url,$order,$order_direction);
			$body_dat['numb_rec_url_left'] =base_url().'index.php/projectslist/index/';
			$body_dat['numb_rec_url_right'] ="/{$order}/{$order_direction}/";	//для формування посилання з кількістю записів на сторінку
						
			$this->load->view('projects-list',$body_dat);//основна таблиця
		}else{				
	...
	public function tasks_project($project_id=NULL)
	{
		if(!empty($project_id)){
			$entered=$this->login->enter();
			if($entered){
				...
				$tasks=$this->projects->TasksOneProj($project_id);	
				echo $this->json->php2js($tasks);
			}
		}
		return FALSE;
	}
